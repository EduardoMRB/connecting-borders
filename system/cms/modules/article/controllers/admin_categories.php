...
class Admin_Categories extends Admin_Controller {
    ...
    public function index() {
        ...
        $total_rows = $this->article_categories_m->count_all();
        $pagination = create_pagination('admin/article/categories/index', $total_rows, NULL, 5);

        // Using this data, get the relevant results
        $categories = $this->article_categories_m->order_by('title')->limit($pagination['limit'])->get_all();
    ...
    public function create() {
        ...
        if ($this->form_validation->run()) {
            if ($id = $this->article_categories_m->insert($_POST)) {
                // Fire an event. A new article category has been created.
                Events::trigger('article_category_created', $id);
    ...
    public function edit($id = 0) {
        // Get the category
        $category = $this->article_categories_m->get($id);

        // ID specified?
        $category or redirect('admin/article/categories/index');

        // Validate the results
        if ($this->form_validation->run()) {
            $this->article_categories_m->update($id, $_POST) ? $this->session->set_flashdata('success', sprintf(lang('cat_edit_success'), $this->input->post('title'))) : $this->session->set_flashdata('error', lang('cat_edit_error'));

            // Fire an event. A article category is being updated.
            Events::trigger('article_category_updated', $id);

            redirect('admin/article/categories/index');
        }
    ...
    public function delete($id = 0) {
        ...
        if (!empty($id_array)) {
            ...
            foreach ($id_array as $id) {
                if ($this->article_categories_m->delete($id)) {
                    $deleted++;
                    $deleted_ids[] = $id;
                } else {
            ...
            Events::trigger('article_category_deleted', $deleted_ids);
        } else {
    ...
    public function _check_title($title = '') {
        if ($this->article_categories_m->check_title($title)) {
    ...
    public function create_ajax() {
        ...
        if ($this->form_validation->run()) {
            $id = $this->article_categories_m->insert_ajax($this->input->post());
